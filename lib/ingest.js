...
export async function ingestCorpus({ documents, firmName, hint }) {
  ...
  // One letter is about four thousand characters, so a fourteen thousand character
  // slice was doing nothing except allowing an unusually long one through whole. Eight
  // is enough to show the structure and keeps the call inside the function's deadline.
  const sample = texts[0].slice(0, 8000);
  ...
  const system = [
    ...
    '',
    'Keep it brief. Give at most six review rules, and one short sentence per message.',
    'Instructions for bespoke sections are one line each, not a paragraph. The structure',
    'is read by code, not by a person, and a long answer is a slow one.',
    '',
