...
export function normaliseDefinition(parsed) {
  const seen = new Set();
  ...
  const finalBlocks = capRequiredFields(demoteOverTemplated(blocks));
  ...
  for (const b of finalBlocks) {
    ...
    for (const m of String(b.body).matchAll(/\{([a-z0-9_]+)\}/gi)) {
      stillNeeded.add(m[1].toLowerCase());
    }
  }
...
export async function ingestCorpus({ documents, firmName, hint }) {
  ...
  const definition = normaliseDefinition(parsed);
