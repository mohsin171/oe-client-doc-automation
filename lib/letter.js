...
export function fixTargetFor(flag, blocks = []) {
  ...
  // A flag that points at no passage has no passage to fix. "This letter was reopened
  // after being signed off" is about the state of the document, and it was being given
  // a Fix Scope Detail button because the keyword fallback scored a block above zero on
  // an incidental word. Sending someone to an unrelated paragraph is worse than sending
  // them nowhere.
  const named = Object.prototype.hasOwnProperty.call(FIX_TARGETS, String(flag?.code || ''));
  if (!flag?.anchor && !named) return null;
...
//
// A client care letter went out reading "How we charge" followed by white space, because
// the paragraph beneath it lost the value it needed and came back empty. The heading
// printed regardless, so the letter appeared to state a charge and stated nothing. For a
// firm that is a regulatory problem rather than a cosmetic one.
//
// A heading belongs to what follows it. If everything between one heading and the next is
// empty, the heading goes too. Applied in both renderers, because a letter on screen and
// a letter downloaded disagreeing about what it says makes an approval mean less.
export function isHeadingBlock(block) {
  if (!block || block.kind !== 'fixed') return false;
  const text = String(block.body || '').trim();
  return text.length > 0 && text.length < 60 && !/[.;:,]$/.test(text);
}

export function dropOrphanedHeadings(blocks = []) {
  const keep = [];
  ...
  for (let i = 0; i < blocks.length; i += 1) {
    ...
    if (isHeadingBlock(block)) {
      // Everything until the next heading is what this one introduces.
      let hasContent = false;
      for (let j = i + 1; j < blocks.length; j += 1) {
        if (isHeadingBlock(blocks[j])) break;
        if (String(blocks[j].body || '').trim()) { hasContent = true; break; }
      }
      if (!hasContent) continue;
    } else if (!String(block.body || '').trim()) {
      // An empty paragraph prints as nothing, so it may as well not be there.
      continue;
    }
