...
import { layoutLetter, dropOrphanedHeadings } from './letter.js';
...
export async function renderLetterPdf({ document: doc, version, firm, salutation, dateText, sender }) {
  ...
  // A heading with nothing under it is dropped along with the empty section it
  // introduces. A letter reading "How we charge" over white space appears to state a
  // charge and states nothing.
  for (const block of dropOrphanedHeadings(parts.body)) {
    ...
    // Compared without punctuation. The block reads "Mr and Mrs D Okonkwo / 17 Grange
    // Croft / Leeds LS17 7EW" across three lines with no commas, and the record holds
    // "17 Grange Croft, Leeds LS17 7EW" with one, so a plain comparison missed it and the
    ...
    const isRecipientBlock = flat.length < 160 && clientName.length > 3
      ...
      && (!clientAddr || flatNoPunct.includes(clientAddr) || flatNoPunct === clientName);
    ...
    const already = isLetterheadLine
      ...
      // The close is drawn below, so a signature block among the body gives two: the
      // fee earner's name and the firm, then Yours sincerely and both again.
      || (sender?.name && flat.length < 90 && flat.includes(String(sender.name).toLowerCase())
          && flat.includes(firmName.toLowerCase()))
      || (sender?.name && flat === String(sender.name).toLowerCase());
    if (already) continue;
