...
// deployment and a wrong model name corrected in a minute. That mattered immediately:
// the first version of this file named claude-sonnet-4-5, which does not exist on the
// account, so every call fell through to the fallback and the upgrade did nothing. The
// console lists Sonnet 5, and it is the model tagged for writing.

const DEFAULTS = {
  // Haiku for a platform reason, not a quality one. Ingestion produces a large JSON
  // structure in one call, Sonnet generates it more slowly, and Vercel Hobby caps a
  // function at 60 seconds, so analysing a corpus began timing out the moment this was
  // upgraded. It is the one job where the model has a hard deadline.
  //
  // Two things make it the least costly place to economise. The counting that decides
  // which clauses are standard is done in code before any model call, so the part that
  // must be right is not the model's to get wrong. And it runs once per template, where
  // a person is present to look at the four numbers and analyse again if they are wrong.
  //
  // Set MODEL_INGEST to claude-sonnet-5 on a plan that allows a longer function, which
