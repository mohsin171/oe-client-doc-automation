...
export async function extractFromNarrative({ narrative, fields, knownValues = {}, firmName }) {
  ...
  const system = [
    ...
    '',
    'Respond with JSON only, no preamble and no code fences. Report only what the notes',
    'say. Do not list what they do not say: an absent field is simply absent and nothing',
    'needs to be written about it.',
    '',
    'Keep each quotation short, twelve words at most. It exists to prove the value came',
    'from the notes, not to reproduce them.',
    '',
    '{"found":[{"key":"...","value":"...","quote":"the exact supporting words"}]}',
  ].join('\n');
