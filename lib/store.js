...
export async function listMatters(firmId) {
  return sql`
...
export async function upsertMatterField(matterId, field) {
  const { key, value, source, provenance, confidence, isNumeric } = field;
...
export async function confirmMatterField(matterId, key, userId) {
  const rows = await sql`
    ...
    WHERE matter_id = ${matterId} AND key = ${key} RETURNING *`;
  return rows[0] || null;
}
...
export async function assessCompleteness(matterId, requiredFields) {
  ...
  const missing = requiredFields.filter((k) => !byKey.has(k));
