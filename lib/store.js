...
export async function assessCompleteness(matterId, requiredFields) {
  ...
  const missing = requiredFields
    ...
    .filter((k) => {
      ...
      // An hourly rate is not missing on a matter agreed at a fixed fee. Requiring both
      // asks a firm to charge two ways at once.
      const values = Object.fromEntries(fields.map((x) => [canonicalKey(x.key), x.value]));
      return !satisfiedByAlternative(k, values);
    });
