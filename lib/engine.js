...
// What a check needs supplying when it cannot be fixed by rewording. Telling
// someone a fact is missing and offering no way to record it leaves them to go
// and find the right screen, which is how a blocking check ends up dismissed
// out of impatience rather than judgement.
const MISSING_FACTS = {
  dangling_estimate: ['fee_estimate', 'hours_estimate'],
  unstated_timescale: ['key_dates', 'completion_date'],
  date_not_past: ['engagement_date'],
};

export function factsNeededFor(code) {
  return MISSING_FACTS[String(code)] || [];
}
...
// ---------------------------------------------------------------
// 5. Suggesting a correction.
//
// Asked for rather than produced automatically. A suggestion nobody wanted
// costs a model call and clutters the screen, and one appearing beside every
// flag invites accepting without reading.
//
// The model proposes wording. It never applies it. That distinction is the
// whole design: a person still decides what the letter says.
// ---------------------------------------------------------------

export async function suggestFix({ block, values, problem, firmName, precedents }) {
  if (!block || !String(block.body || '').trim()) {
    return { ok: false, reason: 'nothing_to_fix' };
  }

  // A standard clause is not the model's to rewrite, even when asked. If the
  // problem is that the letter is missing something the clause refers to, the
  // fix belongs in the passage that should contain it.
  if (block.kind === 'fixed') {
    return { ok: false, reason: 'protected_clause' };
  }
  ...
  const system = [
    `You correct a single passage of a letter for ${firmName}.`,
    '',
    'You are given the passage, the problem with it, and the facts on the file.',
    '',
    'Rules:',
    '1. Use only the supplied facts. If the problem is that something is missing and',
    '   the facts do not contain it, say so rather than inventing it. A suggestion',
    '   that invents a figure is worse than no suggestion at all.',
    '2. Change as little as possible. Correct the problem and leave the rest alone.',
    "3. Keep the firm's register, following the examples if any are given.",
    '4. Return the whole corrected passage, not a description of the change.',
    '',
    'Respond with JSON only, no preamble and no code fences:',
    '{"canFix":true,"suggestion":"the full corrected passage","note":"one line on what changed"}',
    'or',
    '{"canFix":false,"note":"what is missing and who needs to supply it"}',
  ].join('\n');
  ...
  const user = [
    ...
    factLines || '(none)',
    '',
    precedentText ? `How the firm has written this kind of passage before:\n\n${precedentText}\n` : '',
  ...
  if (!parsed.canFix || !String(parsed.suggestion || '').trim()) {
    return {
      ok: true, canFix: false,
      note: String(parsed.note || 'Cannot be fixed from what is on file.'),
    };
  }
