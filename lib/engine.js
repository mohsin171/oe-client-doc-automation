...
import { canonicalKey, satisfiedByAlternative } from './fields.js';
import { modelFor, FALLBACK, isModelMissing } from './models.js';

// Which model does which job lives in lib/models.js, with the reasoning.
...
async function callModel(params, label) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    try {
      ...
      // A model name that is wrong or withdrawn should not stop a firm writing a
      // letter. One retry on the model that is always there, and it is said out loud
      // rather than hidden, because a letter drafted by the fallback is not the letter
      // the firm was promised.
      if (isModelMissing(err) && params.model !== FALLBACK) {
        console.warn(`${label}: ${params.model} unavailable, falling back to ${FALLBACK}`);
        return anthropic.messages.create({ ...params, model: FALLBACK });
      }
      lastErr = err;
    ...
    if (block.kind === 'field') {
      ...
      const body = block.body.replace(PLACEHOLDER, (match, rawName) => {
        ...
        if (v === undefined || v === null || String(v).trim() === '') {
          ...
          // A field answered another way is not missing. This matter agreed a fixed fee
          // of 1,450, so there is no hourly rate and there never will be, and demanding
          // one blocked the letter entirely. The sentence that needed it is dropped, the
          // rest of the paragraph stands.
          if (optional.has(name) || optional.has(rawName)
            || satisfiedByAlternative(name, values)) return '\u0000';
          missing = true;
      ...
      // Two placeholders in one line can resolve to the same value: a template asking
      // for the handler and the fee earner gets one name twice, because the vocabulary
      // makes them the same field. Correct, and it reads as a mistake.
      const collapsed = body
        .replace(/\b([^,;]{2,60}?),\s*\1\b/g, '$1')
        .replace(/\b([A-Z][^,;]{2,60}?)\s+\1\b/g, '$1');

      // A value dropped from the middle of a sentence leaves the sentence broken.
      //
      // "at {hourly_rate} pounds per hour" became "at pounds per hour" on a matter agreed
      // at a fixed fee, and that would have gone to a client over a partner's signature.
      // Removing the value is not enough: the sentence that needed it goes too. If that
      // empties the block, the block is empty and is dropped along with its heading, which
      // is a truthful letter rather than one asserting a charge it does not state.
      // First, the values that can go cleanly. A grade between commas is a parenthetical:
      // "handled by Mohsin Ali, {grade}, who is your contact" reads perfectly without it.
      const commasTidied = collapsed
        .replace(/\s*,\s*\u0000\s*(?=,)/g, '')
        .replace(/\s*,\s*\u0000\s*(?=[.;:]|$)/g, '')
        .replace(/\u0000\s*,\s*/g, '');

      // Whatever marker survives that was not a parenthetical. It sat mid-phrase, so the
      // sentence it was in is broken and goes whole: "at {hourly_rate} pounds per hour"
      // became "at pounds per hour" on a matter agreed at a fixed fee, and that would have
      // reached a client over a partner's signature. If dropping it empties the block, the
      // block is empty and is dropped with its heading, which is a truthful letter rather
      // than one heading a charge it does not state.
      const kept = commasTidied
        .split(/(?<=[.!?])\s+/)
...
export async function draftBespoke({
  blocks, values, precedents, firmName, narrative, history,
}) {
  ...
  // What the file records having happened. A closing letter is largely this, and until
  // now the only way to get it into a letter was a fee earner typing out what the
  // system already knew.
  const account = String(history || '').trim();
  ...
  const system = [
    ...
    '- records any instruction the client has given about how the firm should act. If they',
    '  asked to be contacted only by email and not by post, the letter says so. That is an',
    '  instruction, not a preference, and a letter that ignores it has already failed.',
...
export const VERIFIABLE_CHECKS = new Set([
  ...
  // Not a problem in the letter but a fact about it. Signing off again is what clears
  // it, so offering Resolved would let somebody assert their way past the one thing it
  // is asking for.
  'reopened_after_signoff',
]);
...
export function runDeterministicRules(definition, blocks, values) {
  ...
  // A fee was agreed and the letter states none.
  //
  // A conveyancing matter agreed at a fixed fee of 1,450 produced a letter that mentioned
  // no figure at all, because the firm's template charges hourly, the hourly sentence was
  // dropped for having no rate, and nothing put the fixed fee in its place. The letter was
  // truthful and silent, and a client care letter silent on charges is a regulatory
  // problem rather than an untidy one.
  const agreedFee = ['fixed_fee', 'hourly_rate', 'fee_estimate', 'fee_total']
    .map((k) => String(values[k] ?? '').trim())
    .filter(Boolean);

  if (agreedFee.length > 0) {
    const digits = fullText.replace(/[^0-9]/g, '');
    const stated = agreedFee.some((v) => {
      const d = String(v).replace(/[^0-9]/g, '');
      return d.length >= 2 && digits.includes(d);
    });
    if (!stated) {
      push('fee_not_stated', 'blocking',
        `A fee of ${agreedFee[0]} was agreed and this letter states no figure. The `
        + 'wording it was built from charges a different way, so the sentence that would '
        + 'have carried the fee was dropped. Write the charges paragraph by hand, or use '
        + 'a letter of a kind that matches how this matter is charged.',
        null);
    }
  }

  // A cross-reference to something that is not in the letter.
  //
  // A client care letter said "this fixed fee covers the work set out earlier in this
  // letter", and the work was not set out anywhere: the scope belongs to the engagement
  // letter, not this one. The sentence is fluent, accurate about the fee, and points at
  // nothing. A client following the reference finds no answer, and a firm has told them
  // something is written down when it is not.
  //
  // Advisory rather than blocking. The reference may be to an enclosure, or to a letter
  // sent the same day, and a person can see that in a second where a rule cannot.
  const REFERS_BACK = /\b(?:set out|described|listed|explained|referred to)\s+(?:earlier|above|in this letter)\b|\bas (?:set out|described) (?:above|earlier)\b/i;
  const DESCRIBES_WORK = /\b(?:we will|our work|the work|scope of)\b[^.]{0,80}\b(?:act|include|carry out|advise|comprise|cover)/i;

  for (const b of openBlocks) {
    const text = String(b.body || '');
    if (!REFERS_BACK.test(text)) continue;
    if (DESCRIBES_WORK.test(fullText)) continue;
    push('reference_to_nothing', 'advisory',
      'This refers to something set out earlier in the letter, and the letter does not set '
      + 'it out. A client following the reference finds nothing. Either say it here, or '
      + 'point at the letter that does say it.',
      b.key);
  }
...
export async function generate({
  definition, values, precedents, firmName, docType, optional, narrative, history,
}) {
  ...
  const withBespoke = await draftBespoke({
    blocks: assembled, values, precedents, firmName, narrative, history,
  });

  // A letter with nothing in it must not be produced.
  //
  // An engagement letter came out with no scope, no charges and no exclusions. Every
  // section particular to the matter was empty, the empty sections were dropped along
  // with their headings, and what remained was boilerplate that read perfectly and said
  // nothing about the client at all. Dropping an empty section is right; doing it
  // silently and calling the remainder a letter is not.
  //
  // So this is checked before anything else. The sections a letter is written for are the
  // bespoke ones, and if they are empty the drafting did not happen, whatever the reason:
  // a model that returned nothing, a reply that would not parse, a call that timed out.
  // A person should be told that and asked to try again, not handed two pages of
  // boilerplate with the substance quietly removed.
  const bespokeBlocks = withBespoke.filter((b) => b.kind === 'bespoke');
  const written = bespokeBlocks.filter((b) => String(b.body || '').trim().length > 20);
  if (bespokeBlocks.length > 0 && written.length === 0) {
    return {
      ok: false,
      reason: 'nothing_drafted',
      detail: `None of the ${bespokeBlocks.length} sections this letter is written for came `
        + 'back. The rest of it is standard wording, so producing it would give you a '
        + 'letter that says nothing about this matter.',
    };
  }
