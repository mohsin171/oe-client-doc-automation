...
export async function draftBespoke({ blocks, values, precedents, firmName, narrative }) {
  ...
  const system = [
    ...
    '2. Write in the voice and register of the supplied precedents. Take wording from them,',
    '   never content. Every figure, date, name, address and party in a precedent belongs to',
    '   a different client and is wrong for this matter. If a precedent says the fee is 1,750',
    '   and the facts below say 2,400, the answer is 2,400. If the facts do not give a figure',
    '   at all, write no figure: a precedent is not a source of facts.',
  ...
  const user = [
    ...
    notes
      ? [
        ...
        'conversation this letter is about, and the letter should reflect it. Where it',
        'gives a detail the facts above do not carry, use it. Where it and the facts',
        'disagree, the facts are right, because a person has confirmed them.',
        '',
        '---',
        notes,
        '---',
    ...
    '',
    precedentText
      ? 'The firm\'s own past wording, to match in tone. These belong to other clients: '
        + 'the phrasing is useful, the contents are not.\n\n' + precedentText
      : '',
    '',
    'Sections to draft:',
